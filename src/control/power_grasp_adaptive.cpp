...
namespace control{
    namespace ad_control{
        ...
        Eigen::VectorXd ADPowerGrasp::joint_space_control(const size_t& ind,const Eigen::VectorXd& target_pos){
            ...
            psi_d.segment(ad_hsize,ad_hsize) = -4. *(psi.segment(0,ad_hsize) - psi_d.segment(0,ad_hsize)) ;
            if(psi_d.segment(ad_hsize,ad_hsize).norm() > .5)
                psi_d.segment(ad_hsize,ad_hsize) = .5 * psi_d.segment(ad_hsize,ad_hsize).normalized();
        ...
        void ADPowerGrasp::algorithm(){
            ...
            if(_step == 0){
                Eigen::VectorXd adGains = Eigen::VectorXd::Constant(4,0.3);
                adGains(1) *= 1.;
                adGains(2) *= 1.;
                adGains(3) *= 1.;
                // Eigen::Vector4d imp = Eigen::Vector4d(0.1, 0.1, 0.1, 0.05);
            ...
            //         do_grasp = false;
            //         for (size_t i = 0; i < _AD.size(); i++){
            //             _AD[i]->deactivateImpedance();
            //             _AD[i]->resetAdaptation();
            //         }
            //     }
            // }else if(_step > 5*_sc+200){
            //     do_grasp = true;
            // }else if(_step > 200){
            //     if (do_grasp == true)
            //     {
            ...
            if(do_grasp){
                ...
                targetJointPositions[3] =  Eigen::Vector4d(1.4,0.1, -0.15, 0.1);
            }

            
            //*******get force from passie Ds
            Eigen::MatrixXd A = Eigen::MatrixXd::Identity(4,4);
            A.diagonal() << 1.5, 1., 0.8, 0.5;
            A.diagonal() *=1; 
            for (size_t i = 0; i < numfingers ; i++){                
                ...
                if(do_grasp){
                    ...
                    _AD[i]->deactivateImpedance();
                    _AD[i]->resetAdaptation();
                    Eigen::VectorXd pos = std::static_pointer_cast<robot::Hand>(robots[0])->finger[i].joint_states.pos;
                    Eigen::VectorXd joint_vel = std::static_pointer_cast<robot::Hand>(robots[0])->finger[i].joint_states.vel;

                    Eigen::VectorXd _er = targetJointPositions[i] - pos;
                    for(size_t j=0;j<_er.size();j++){
                        if(std::abs(_er[j]) > 0.3){
                            _er[j] *= 0.3/std::abs(_er[j]);
                        }
                    }

                    cmdTorque.segment(4*i,4) = A * (_er -0.1*joint_vel);
                }
